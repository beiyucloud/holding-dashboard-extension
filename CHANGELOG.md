...
- **修复 v55 图表 setOption 引用未定义变量 `chart` 导致图表完全无数据**：v55 图表内存泄漏重构时本应把所有对 `chart` 的引用改为模块级 `chartInstance`，但 `renderChart` 第 604 行漏改，仍在 `chart.setOption({...})`（`chart` 永未定义）——`renderChart` 抛 `ReferenceError` 被外层 catch 接住、递归调用 `renderChart([], [], [], null, null)` 又同样抛错，图表区只剩 legend 文字 + 底部虚线、曲线和数据点全无。修复：第 604 行改 `chartInstance.setOption({...}, true)`（同时补回 v55 漏掉的 `notMerge` 参数），图表正常渲染。
### 优化（代码质量）
- **修复图表内存泄漏**：`renderChart` 每次 `refreshAll()` 都重新 `echarts.init` 同一 DOM 并新增一个 `resize` 监听，旧实例不 dispose、监听器持续累积（长时间定时刷新会内存泄漏，且 ECharts 对同 DOM 重复 init 会打警告）。改为模块级 `chartInstance` 复用，仅在首次 init 时绑定一次 `resize` 监听，`setOption` 用 notMerge 模式完全替换旧配置。
...
- **公告弹窗新增「速读卡片」**：原弹窗打开后只能直接看动辄 800 字的法律套话正文（"本公司及董事会全体成员保证信息披露内容的真实、准确和完整..."），用户要从大段套话里翻出"每10股派X元" / "回购金额" / "审议事项"等核心要点，体感差。改进：从标题 + 正文中提炼「类型标签 + 3-4 条关键数字/事件」，渲染到正文之上成为一张浅蓝渐变速读卡片（如"分红派息 / 每 10 股派 0.50 元（含税）/ 合计派现 2,085 万元 / 基数 4.17 亿股"），完整正文依旧可滚到下方查阅，原文 PDF 链接不变。纯本地规则化提取（无网络依赖、零失败处理），覆盖 17 种常见公告类型（分红派息 / 股份回购 / 股东减持 / 股东增持 / 业绩预告 / 重大合同 / 股东会决议 / 董事会决议 / 股权激励 / 重大资产 / 关联交易 / 停牌 / 复牌 / 风险提示 / 高管变动 / 解禁 / 现金管理）；未识别类型或提取不出要点的公告，整卡片自动隐藏，弹窗行为完全不变。新增 `noticeSummarizer` IIFE（`summarize(title, html)`）+ `renderNoticeSummary` 渲染 helper + `dashboard.html` 加 `.notice-summary` 样式与 `#noticeSummary` 节点。

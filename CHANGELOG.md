...
- **修复速读卡片在董事会决议公告上漏渲染 + 抓错议案**（v59 翠微股份 AN202608281828627638 实测踩坑，commit `c0f0abc`）：① `detectTag` 加标题强信号——`/董事会.{0,6}决议公告|第N届董事会/` 锁定董事会决议、`/股东会.{0,6}决议公告|第N届股东会/` 锁定股东会决议，先于打分；正文里"聘任公司总经理"等高权重关键词不再把决议类型挤掉。② `stripHtml` 正则收紧——`<[^>]+>` 改 `<(?:\/?[a-zA-Z][^>]*?|!--[\s\S]*?--|\?[^]*?\?)>`，只去真正 HTML 标签，不再误吃东财正文里的 `<公司章程>` 这种章节标题括号（导致 "《关于增加经营范围暨修订<公司章程>的议案》" 被提取成 "《关于增加经营范围暨修订 的议案》"）。③ 决议分支优先抓 `/审议通过[了]?《([^》\n]{2,40})》/g` 紧跟的《议案名》——精准区分"议案 vs 正文里的法律引用《公司法》《公司章程》"；若无再兜底抓审议通过短句、再兜底所有《》。④ 决议类公告 items 上限放宽到 6 条（其他类型保持 4 条）——翠微股份这条命中 6 条真议案：半年报 / 聘任高管 / 补选董事 / 补选委员会 / 修订章程 / 修订对外担保制度。

> 发布状态：已随 **v1.0.7** 发布——GitHub `main` + tag `v1.0.7` + Release（含 zip）；官网 beiyucloud.taoxinyuan.com 同步上线；Edge 加载项商店提交审核。

- **速读卡片新增 4 类公告支持**（v60 翠微股份 AN202608281828627627/639/640/632/642/647 实测补全）：用户截图「提醒记录」里有半年报 / 半年报摘要 / 股东会通知 / 经营数据公告 / 经营范围变更 / 公司章程 这 6 类公告之前没出卡片，本次扩展。① detectTag 加 4 条规则——定期报告（年报/半年报/季报/中期报告/经营数据公告）、股东会通知（关于召开…次…股东会的通知）、经营范围变更、公司章程（带括号修订/草案）。② extractItems 加 4 个分支：定期报告按"营业收入 / 利润总额 / 归母净利润 / 经营性现金流 + 同比"通用模板抓，半年度报告摘要额外补"总资产/归母净资产（较上年末%）"；经营数据公告抓"主营合计（同比%）"；股东会通知抓"召开日期 / 召开时间 / 股权登记日 / 召开地点"（中间夹"Ａ股 603123 翠微股份"列名放宽到 30 字符内找日期）；经营范围变更抓"修订条款第 N 条"+"涉及：经营范围/对外担保"+"尚需股东会审议"；公司章程抓章节数。③ stripHtml 多处理 `&minus;` / `&mdash;` / `&middot;` 三个常见实体（表格里负号常写为 `&minus;`，否则抓不到数字）。4 类共提炼出 6+ 条测试样例，全部通过端到端真实公告验证（翠微股份半年报命中 4 条财务数据 + 同比；股东会通知命中 4 条关键日期地点；经营数据公告命中"主营合计 933M 元 同比 -13.79%"；经营范围变更命中"修订条款第 15 条、第 46 条"+"涉及：经营范围/对外担保"）。

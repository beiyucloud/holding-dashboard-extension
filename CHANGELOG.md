...
- **修复速读卡片在董事会决议公告上漏渲染 + 抓错议案**（v59 翠微股份 AN202608281828627638 实测踩坑）：① `detectTag` 加标题强信号——`/董事会.{0,6}决议公告|第N届董事会/` 锁定董事会决议、`/股东会.{0,6}决议公告|第N届股东会/` 锁定股东会决议，先于打分；正文里"聘任公司总经理"等高权重关键词不再把决议类型挤掉。② `stripHtml` 正则收紧——`<[^>]+>` 改 `<(?:\/?[a-zA-Z][^>]*?|!--[\s\S]*?--|\?[^]*?\?)>`，只去真正 HTML 标签，不再误吃东财正文里的 `<公司章程>` 这种章节标题括号（导致 "《关于增加经营范围暨修订<公司章程>的议案》" 被提取成 "《关于增加经营范围暨修订 的议案》"）。③ 决议分支优先抓 `/审议通过[了]?《([^》\n]{2,40})》/g` 紧跟的《议案名》——精准区分"议案 vs 正文里的法律引用《公司法》《公司章程》"；若无再兜底抓审议通过短句、再兜底所有《》。④ 决议类公告 items 上限放宽到 6 条（其他类型保持 4 条）——翠微股份这条命中 6 条真议案：半年报 / 聘任高管 / 补选董事 / 补选委员会 / 修订章程 / 修订对外担保制度。
